...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name == "lower" || func_name == "upper") {
    if (args.size() != 1) {
      //      throw Exception(fmt::format("Please input the correct args for {}",func_name));
      throw Exception(fmt::format("error"));
    }
    ...
    throw Exception(fmt::format("The func {} you input is even not exists!!", func_name));
  }
}
